...
export default function MenuPage({ params }: { params: { slug: string } }) {
  ...
  const categoriesQuery = useQuery({
    ...
    queryFn: () =>
      api.get<MenuCategory[]>(`/api/admin/menu/categories?tenant_id=${tenantId}`),
  });

  const itemsQuery = useQuery({
    queryKey: ["menu-items", tenantId],
    queryFn: () => api.get<MenuItem[]>(`/api/admin/menu/items?tenant_id=${tenantId}`),
  });
  ...
  const updateCategory = useMutation({
    mutationFn: (payload: { id: number; name: string; sort_order: number; active: boolean }) =>
      api.put<MenuCategory>(
        `/api/admin/menu/categories/${payload.id}?tenant_id=${tenantId}`,
        {
          name: payload.name,
  ...
  const deactivateCategory = useMutation({
    mutationFn: (categoryId: number) =>
      api.delete<MenuCategory>(
        `/api/admin/menu/categories/${categoryId}?tenant_id=${tenantId}`
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["menu-categories", tenantId] });
    },
  ...
  const deactivateItem = useMutation({
    mutationFn: (itemId: number) =>
      api.delete<MenuItem>(`/api/admin/menu/items/${itemId}?tenant_id=${tenantId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["menu-items", tenantId] });
    },
  });
