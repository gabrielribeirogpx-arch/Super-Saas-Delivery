...
export default function InventoryPage({ params }: { params: { tenantId: string } }) {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["inventory", tenantId, fromDate, toDate],
    queryFn: async () => {
      const [items, movements] = await Promise.all([
        api.get<InventoryItem[]>(`/api/inventory/items?tenant_id=${tenantId}`),
        api.get<InventoryMovement[]>(
          `/api/inventory/movements?tenant_id=${tenantId}&de=${fromDate}&para=${toDate}`
        ),
      ]);
