...
export default function KdsPage({ params }: { params: { tenantId: string } }) {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["kds", tenantId, area],
    queryFn: () =>
      api.get<KdsOrder[]>(`/api/kds/orders?tenant_id=${tenantId}&area=${area}`),
    refetchInterval: 5000,
  });

  const startMutation = useMutation({
    mutationFn: (orderId: number) =>
      api.post(`/api/kds/orders/${orderId}/start?tenant_id=${tenantId}&area=${area}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["kds", tenantId, area] }),
  });

  const readyMutation = useMutation({
    mutationFn: (orderId: number) =>
      api.post(`/api/kds/orders/${orderId}/ready?tenant_id=${tenantId}&area=${area}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["kds", tenantId, area] }),
  });
